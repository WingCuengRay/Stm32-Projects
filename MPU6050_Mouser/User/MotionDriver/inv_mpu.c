...
#define i2c_write   AnBT_DMP_I2C_Write
#define i2c_read    AnBT_DMP_I2C_Read
...
unsigned char DMP_MPU6050_DEV_CFG(void)		
{
	  unsigned char anbt_dmp_data[6], anbt_dmp_rev;
		//检验ＷＨＯ　ＡＭ　Ｉ
		if (i2c_read(st.hw->addr, st.reg->who_am_i, 1, &(anbt_dmp_data[0])))
			return 1;  
	  if (anbt_dmp_data[0]!=0x68) 
		{
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0])))
			return 3;   
    delay_ms(100);
		//
    ...
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(anbt_dmp_data[0])))
			return 4;
    //
    if (i2c_read(st.hw->addr, st.reg->accel_offs, 6, anbt_dmp_data))
    ...
    if (anbt_dmp_rev)  //Congrats, these parts are better
		...
		{
			if (i2c_read(st.hw->addr, st.reg->prod_id, 1, &(anbt_dmp_data[0]))) return 7;      
